...
// krkrz 版本漂移适配：
// 1) TJS_NCM_REG_THIS 默认为 this，但 KAGParser 的 TVPCreateNativeClass_KAGParser
//    是类外工厂函数（仅有局部 classobj 变量），需要 classobj 分支。
//    不通过 __TP_STUB_H__ 启用（会触发 tTJSNativeClassForPlugin 重定义），改为直接覆盖宏。
// 2) TVPInternalError 在新 krkrz 中改为 tTJSMessageHolder（MsgIntfInc.h 声明），
//    删除此处旧式 const tjs_char* 定义以避免类型冲突。
// 3) TJS_NATIVE_SET_ClassID 在非 __TP_STUB_H__ 下硬编码 ClassID，但工厂函数无此
//    成员；覆盖为用 KAGParser 自己的 ClassID_KAGParser 静态变量。
