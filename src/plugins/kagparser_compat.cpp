...
/* KAGParser source: Copyright (C) 2000 W.Dee and contributors (Kirikiri license, see license.txt) */
/* Integrated into krkrsdl2 web build to provide the KAGParser native class. */
...
// Use the Ex-plugin path: tTJSNC_KAGParser becomes a struct with a static
// CreateNativeClass() that returns a tTJSNativeClassForPlugin (= iTJSDispatch2).
// TVP_KAGPARSER_EX_PLUGIN disabled: Ex-plugin static CreateNativeClass path needs
// __TP_STUB_H__ which redefines tjsNative.h real classes. Use constructor path (like menuitem_compat).
// #define TVP_KAGPARSER_EX_PLUGIN

// Headers that the non-EX KAGParser.cpp would have included itself; in EX mode
// (TVP_KAGPARSER_EX_PLUGIN defined) it skips its own include block, so provide them.
#include "KAGParser.h"
...
// KAGParser error message strings (referenced via TVP_KAGPARSER_MESSAGEMAP).
static const tjs_char* TVPKAGNoLine = TJS_W("line does not exist in scenario %1");
static const tjs_char* TVPKAGCannotOmmitFirstLabelName = TJS_W("first label name cannot be omitted");
static const tjs_char* TVPKAGMalformedSaveData = TJS_W("save data is corrupted");
static const tjs_char* TVPKAGLabelNotFound = TJS_W("label %2 not found in scenario %1");
static const tjs_char* TVPLabelOrScriptInMacro = TJS_W("label or iscript cannot be described in macro");
static const tjs_char* TVPKAGInlineScriptNotEnd = TJS_W("[endscript] or @endscript not found");
static const tjs_char* TVPKAGSyntaxError = TJS_W("KAG syntax error");
static const tjs_char* TVPKAGCallStackUnderflow = TJS_W("return tag without matching call tag");
static const tjs_char* TVPKAGReturnLostSync = TJS_W("scenario changed; cannot return to previous position");
static const tjs_char* TVPKAGSpecifyKAGParser = TJS_W("KAGParser object must be specified");
static const tjs_char* TVPUnknownMacroName = TJS_W("macro not registered");
...
{
	static bool methodInitialized = false;
...
// ---- Registration as an integrated plugin (mirrors menuitem_compat.cpp) ----
static iTJSDispatch2 * TVPCreateNativeClass_KAGParser_Wrapped(iTJSDispatch2 *global)
{
	(void)global;
#ifdef TVP_KAGPARSER_EX_PLUGIN
	iTJSDispatch2 *cls = tTJSNC_KAGParser::CreateNativeClass();
#else
	iTJSDispatch2 *cls = new tTJSNC_KAGParser();
#endif
	TVPAddImportantLog(TJS_W("(info) Installing KAGParser (web integrated plugin)"));
	return cls;
}

static tTVPAtInstallClass TVPInstallClassKAGParser(
	TJS_W("KAGParser"), TVPCreateNativeClass_KAGParser_Wrapped, TJS_W(""));
